...
public class AssessmentCandidateSpecification: SpecificationBase<Assessment.Assessment>
{
    private readonly List<int> _assessmentIds = new();

    public AssessmentCandidateSpecification(IEnumerable<int> assessmentIds)
    {
        _assessmentIds = assessmentIds?.ToList() ?? _assessmentIds;
    }
    
    public override Expression<Func<Assessment.Assessment, bool>> Criteria 
        => t => !_assessmentIds.Any() || _assessmentIds.Contains(t.UserId);
}